...
const TechnicianInfo = ({ route, navigation }) => {
    ...
    useEffect(() => {
      const loadProfile = async () => {
        try {
          const response = await profileServices.fetchTechnicianProfile(technicianId);         
          setProfile(response);
        } catch (err) {
  ...
  !profile?.profile_image_url
    ...
    const handleDesignPress = async (design) => {
        try {
          const fullDesign = await savedDesignsApi.fetchDesignDetail(design.id);
          navigation.navigate("DesignDetail", { design: fullDesign });
        } catch (error) {
          console.error("Failed to load full design:", error);
    ...
    <SafeAreaView style={authStyles.safeArea}>
      ...
      {loading ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading technician profile...</Text>
        </View>
      ) : (
