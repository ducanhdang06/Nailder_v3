...
const CustomerChat = ({ navigation }) => {
  ...
  // Fetch user chats (temporarily disabled)
  const { data, loading, error, refetch } = useQuery(GET_USER_CHATS, {
    variables: { user_id: userId },
    skip: !userId || !ENABLE_CHAT_QUERY, // Skip query until resolver is ready
    errorPolicy: 'all',
    fetchPolicy: 'cache-and-network',
    pollInterval: ENABLE_CHAT_QUERY ? 5000 : 0, // Disable polling when query is disabled
    onError: (apolloError) => {
  ...
  const renderChatItem = ({ item, index }) => {
    const isCustomer = item.customer_id === userId;
    const otherUser = isCustomer ? item.technician : item.customer;
    ...
    return (
      <SafeAreaView style={authStyles.safeArea}>
        <View style={uploadStyles.pageHeader}>
          <Text style={uploadStyles.pageHeaderTitle}>Messages</Text>
        </View>
      ...
      <View style={styles.container}>
        {chats.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtitle}>
              Start chatting with nail technicians about designs you love!
            </Text>
          </View>
        ) : (
          <FlatList
