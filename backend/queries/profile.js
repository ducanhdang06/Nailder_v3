const getTechnicianProfileQuery = `
  SELECT 
    ...
    (
      ...
      FROM (
        SELECT id, title, image_url, likes
        FROM designs
        WHERE tech_id = u.id
        ORDER BY likes DESC
        LIMIT 3
  ...
  GROUP BY u.id, tp.tech_id, tp.bio, tp.phone_number, tp.location, tp.profile_image_url, tp.social_links, tp.years_experience, tp.specialties
`;
